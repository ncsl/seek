...
class Timeseries(object):
    ...
    def time_line(self):
        return numpy.arange(
            self.time_start, self.end_time + self.time_step, self.time_step
        )
    ...
    def _get_index_of_state_variable(self, sv_label):
        try:
            sv_index = self.dimension_labels[
            ...
            self.logger.error(
                ...
                % (
                    sv_label,
                    self.dimension_labels[TimeseriesDimensions.STATE_VARIABLES.value],
                )
            )
            raise
        return sv_index
    ...
    def get_state_variable(self, sv_label):
        ...
        return Timeseries(
            numpy.expand_dims(sv_data, 2),
            OrderedDict(
                {
                    TimeseriesDimensions.SPACE.value: self.dimension_labels[
    ...
    def lfp(self):
        if (
            TimeseriesDimensions.STATE_VARIABLES.value
            not in self.dimension_labels.keys()
        ):
            self.logger.error("No state variables are defined for this instance!")
            raise ValueError

        if (
            PossibleStateVariables.LFP.value
            in self.dimension_labels[TimeseriesDimensions.STATE_VARIABLES.value]
        ):
            return self.get_state_variable(PossibleStateVariables.LFP.value)
        if (
            PossibleStateVariables.X1.value
            in self.dimension_labels[TimeseriesDimensions.STATE_VARIABLES.value]
            and PossibleStateVariables.X2.value
            in self.dimension_labels[TimeseriesDimensions.STATE_VARIABLES.value]
        ):
            self.logger.info(
                "%s are computed using %s and %s state variables!"
                % (
                    PossibleStateVariables.LFP.value,
                    PossibleStateVariables.X1.value,
    ...
    def _get_indices_for_labels(self, list_of_labels):
        ...
        for label in list_of_labels:
            try:
                space_index = self.dimension_labels[
                    TimeseriesDimensions.SPACE.value
                ].index(label)
            except ValueError:
                self.logger.error(
    ...
    def get_subspace_by_index(self, list_of_index):
        ...
        subspace_dimension_labels = deepcopy(self.dimension_labels)
        subspace_dimension_labels[TimeseriesDimensions.SPACE.value] = numpy.array(
    ...
    def decimate_time(self, time_step):
        ...
        return Timeseries(
            time_data, self.dimension_labels, self.time_start, time_step, self.time_unit
        )
