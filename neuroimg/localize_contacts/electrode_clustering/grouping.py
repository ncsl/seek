...
class Cluster:
    ...
    def find_clusters(self, maskedCT, pointsOnly=True):
        ...
        -------
            allclusters: dict()
                Dictionary of clusters found at various thresholds. The keys are
                threshold values; the values are dictionaries of clusters with
                their corresponding list of coordinates
                of points in each clsuter.

            numobj: List[int]
                List of number of clusters found for the range of thresholds
        """
        ...
        for i in range(len(threshvec)):
            ...
            if pointsOnly is True:
                # filter out all background, i.e. zero-valued, pixels in
                # cluster_labels array tuple of arrays, one for each dim,
                # containing coordinates of nonzero values
                ...
                for j in range(len(nonzero_voxs)):
                    # Check if coordinate in brainmask is non-zero,
                    # i.e. within the brain
...
class CylindricalGroup:
    ...
    def points_in_cylinder(self, pt1, pt2, r, points_to_test):
        """
        Tests whether a list of points lies within a cylinder with points pt1
        and pt2 that define the axis of the cylinder and a specified radius r.
    ...
    def cylinder_filter(self, elec_in_brain, clusters, radius):
        ...
        -------
            ...
            into a cylinder.
        """
        cluster_points = copy.deepcopy(clusters)
        ...
        # In case the channel labels were not in order, we now sort
        # them in order and then consider only the most medial and
        # surface-level contacts.
        for electrode in group_channel_labels:
            labels = group_channel_labels[electrode]
            numlabels = len(labels)
            if numlabels < 2:
                raise IndexError(f"Too few labels for electrode {electrode}")
            first_num = labels[0]
            last_num = labels[-1]
